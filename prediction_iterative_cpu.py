...
parser.add_argument('-d', '--date', help='specify the date till which the input will be considered, forecast for next day and so on will be plotted, format=YYYY-MM-DD', required=True)
parser.add_argument('-n', '--num_forecasts', help='specify the number of days to forecast(int)')
...
seq_channels_last.add(ConvLSTM2D(filters=4, kernel_size=(3, 3), padding='same', 
                                  input_shape=(None, 129, 135, len(EXT_VARIABLES) + 1),
...
try:
    model_path=os.path.join(seq_path, f'ld{LEADTIME}_{len(EXT_VARIABLES)}extvars_{var_string}.h5')
    # seq = load_model(os.path.join(seq_path, f'ld{LEADTIME}_{len(EXT_VARIABLES)}extvars_{var_string}.h5')) for gpu
...
dg_test=datagen(dat, batch_size=1, leadtime=LEADTIME, in_frames=5, max=T, ext_variables=EXT_VARIABLES, mode="forecast", data_format="channels_last") #to be used as input for prediction
...
for i in range(1, int(num_forecasts)+1):
  ...
  if i!=1:
    ...
    dg_test=datagen(ds, batch_size=1, leadtime=LEADTIME, in_frames=5, max=T, ext_variables=EXT_VARIABLES, mode="forecast", data_format="channels_last") 
    dat=ds
