...
async def results(request: Request):
    ...
    return templates.TemplateResponse("results2.html", {"request": request, "encoded_plots": encoded_plots})
...
async def predict(request: Request, date: str = Form(...), num_forecasts: int = Form(...), ext_variables: list = Form([])):
    # ...
    ...
    # Model Prediction
        ...
        validpts2 = np.where(y>=0)
        y[validpts2] = np.exp(y[validpts2])#
        y[validpts2] = np.clip(np.power(y[validpts2], 7), a_min=None, a_max=1e10)
        y[undefpts2] = 0#
        ...
        plot_bytes = BytesIO()
        plt.imshow(yhat[0, 0, :, :], cmap='nipy_spectral_r', vmin=0, vmax=50)
        ax1 = plt.axes()
        ...
        # plt.title(f'Lead Day {num_forecasts} Prediction')
